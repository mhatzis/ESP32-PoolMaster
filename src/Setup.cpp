...
{ 
  ...
  2700, 2700, 30000,
  5000, 3600000, 0, 0,
  8.2, 720.0, 1.8, 0.47, 10.0, 18.0, 3.0, 3.760000, 0.0048662, -876.430775, 2328.898500, 1.0, 0.0,
  2400000.0, 0.0, 0.0, 18000.0, 0.0, 0.0, 0.0, 0.0, 28.0, 7.3, 720., 1.3,
...
ConfigVersion
Ph_RegulationOnOff, Orp_RegulationOnOff, AutoMode, WinterMode
...
Ph_SetPoint, Orp_SetPoint, PSI_HighThreshold, PSI_MedThreshold, WaterTempLowThreshold, WaterTemp_SetPoint,TempExternal, pHCalibCoeffs0, pHCalibCoeffs1, OrpCalibCoeffs0, OrpCalibCoeffs1, PSICalibCoeffs0, PSICalibCoeffs1
Ph_Kp, Ph_Ki, Ph_Kd, Orp_Kp, Orp_Ki, Orp_Kd,PhPIDOutput, OrpPIDOutput, TempValue, PhValue, OrpValue, PSIValue);
...
//Pump PhPump(PH_PUMP, PH_PUMP, NO_TANK,NO_INTERLOCK);
Pump PhPump(PH_PUMP, PH_PUMP, FILTRATION_PUMP, NO_TANK,  NO_INTERLOCK, storage.pHPumpFR, storage.pHTankVol);
Pump ChlPump(CHL_PUMP, CHL_PUMP, NO_LEVEL, FILTRATION_PUMP, storage.ChlPumpFR, storage.ChlTankVol, storage.ChlFill);
Pump RobotPump(ROBOT_PUMP, ROBOT_PUMP, FILTRATION_PUMP);
...
void setup()
{
  ...
  // As the relays on the board are activated by a HIGH level, set all levels LOW at startup
  digitalWrite(FILTRATION_PUMP,LOW);
  digitalWrite(PH_PUMP,LOW); 
  digitalWrite(CHL_PUMP,LOW);
  digitalWrite(ROBOT_PUMP,LOW);
  digitalWrite(RELAY_R0,LOW);
...
void StartTime(){
  ...
  configTime(0, 0,"0.pool.ntp.org","1.pool.ntp.org","212.18.3.19"); // 3 possible NTP servers
...
void readLocalTime(){
  if(!getLocalTime(&timeinfo)){
    ...
    Serial.print("Setting time from RTC : ");
    ...
    Serial.println();
    setTime(now.year()-100,now.month()+1 ,now.day(),now.hour(),now.minute(),now.second());
...
void syncToTimeProvider() {
  ...
  Serial.print("current esp set time is: ");
  Serial.println(&timeinfo,"%A, %B %d %Y %H:%M:%S");
getLocalTime(&timeinfo);
  unsigned long yr = timeinfo.tm_year + 1900;
  unsigned long mt = timeinfo.tm_mon + 1;
  unsigned long dy = timeinfo.tm_mday;
  unsigned long hr = timeinfo.tm_hour;
  unsigned long mi = timeinfo.tm_min;
  unsigned long se = timeinfo.tm_sec;

  rtc.adjust(DateTime(yr, mt, dy, hr, mi, se));
  //rtc.adjust((timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec,timeinfo.tm_mday,timeinfo.tm_mon+1,timeinfo.tm_year-100));
  //rtc.adjust(DateTime(timeinfo.tm_mday,,timeinfo.tm_mon+1,timeinfo.tm_year-100,timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec));
...
DateTime now = rtc.now();
  ...
  {
      Serial.print("Using local Time as no ntp server found");
     DateTime now = rtc.now();
     Serial.print(now.year(), DEC);
    Serial.print('/');
    Serial.print(now.month(), DEC);
