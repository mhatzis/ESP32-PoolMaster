  ...
  for(;;) {
    ...
    {  
      ...
      else
      {
        ...
        else if (command.containsKey(F("RobotPump"))) //"RobotPump" command which starts or stops the salt Chlorinator
        {
          if ((int)command[F("RobotPump")] == 0){
            RobotPump.Stop();    //stop chlorinator
          } else {
            RobotPump.Start();   //start chlorinator
          }  
        }
        ...
        else if (command.containsKey(F("Relay")))
        {
          ...
          {
            case 0:
              (bool)command[F("Relay")][1] ? digitalWrite(RELAY_R0, HIGH) : digitalWrite(RELAY_R0, LOW);
              break;
          }
        }
