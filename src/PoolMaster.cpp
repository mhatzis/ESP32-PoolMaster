...
void PoolMaster(void *pvParameters)
{
  ...
  {  
    ...
    //start chlorinator for 6 hours, 1min after filtration start
    if (FiltrationPump.IsRunning() && storage.AutoMode && !storage.WinterMode && !RobotPump.IsRunning() &&
        ((millis() - FiltrationPump.LastStartTime) / 1000 / 60) >= 1 && !cleaning_done)
    {
        RobotPump.Start();
        Debug.print(DBG_INFO,"Chlorinator Start 1min after Filtration");    
    }
    if(RobotPump.IsRunning() && storage.AutoMode && ((millis() - RobotPump.LastStartTime) / 1000 / 60) >= 360)
    {
        RobotPump.Stop();
        cleaning_done = true;
        Debug.print(DBG_INFO,"Chlorinator Stop after: %d min",(int)(millis()-RobotPump.LastStartTime)/1000/60);
    }
    ...
    if (storage.AutoMode && FiltrationPump.IsRunning() && !AntiFreezeFiltering && (hour() >= storage.FiltrationStop || hour() < storage.FiltrationStart))
    {
        ...
        SetOrpPID(false);
        FiltrationPump.Stop();
        RobotPump.Stop();//clorinator stop
    }
...
void SetPhPID(bool Enable)
{
  if (Enable)
  {
    ...
    storage.Ph_RegulationOnOff = 1;
     Serial.println("phpid start with clear errors**********>>>>>>>>>>>>>>>>>");
  }
  else
  {
    //Stop PhPID
